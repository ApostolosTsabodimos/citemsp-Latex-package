...
\usepackage{citemsp}
\renewcommand{\citemspscale}{0.5} %adjust locators size
...
\texttt{\textbackslash autocite[§3.2]\{smith2003\}}
  & $\longrightarrow$ & \autocite[§3.2]{smith2003}\\[3pt]
\texttt{\textbackslash autocites[§3.2]\{smith2003\}[¶2]\{meyerrochow2003\}}
  & $\longrightarrow$ & \autocites[§3.2]{smith2003}[¶2]{meyerrochow2003}
\end{tabular*}
\end{tcolorbox}
%
\noindent This approach has two primary limitations:

%\begin{enumerate}[leftmargin=2em]
%  \item \textbf{Verbose syntax.}  Citing multiple sources with locators
%    requires nested optional arguments, which is both cumbersome in source files and impractical in the compiled document.
%  \item \textbf{No typed locators.}  There is no efficient and compact notation for
%    referencing specific equations, figures, definitions, theorems etc. within cited literature. 
%\end{enumerate}

\begin{enumerate}[leftmargin=2em]
  \item \textbf{Verbose syntax.} Each additional locator requires a full
    optional-argument pair, both in the source (\texttt{\textbackslash
    autocites[...]\{key\}[...]\{key\}...}) and in the compiled output, where
    the postnote is typeset as plain bracketed text indistinguishable from
    the citation number.  As the example above shows,
    the bracket grows linearly with every additional key and locator,
    quickly cluttering the running text.
\item \textbf{No registry, in either backend.} \pkg{natbib} and \pkg{biblatex}
  are the two general-purpose citation backends used by nearly all modern LaTeX documents. \pkg{natbib}'s postnote is free text
  with no type system at all. \pkg{biblatex} comes closer with a
  \texttt{pagination} field, but it recognizes only a small, localized set
  of generic types (page, chapter, section, paragraph, verse, line, column),
  is fixed once per bibliography entry rather than chosen per citation, and
  excludes scholarly types such as equation, theorem, figure, or lemma.
  \end{enumerate}

\noindent \pkg{citemsp} addresses both limitations directly while working on top of \pkg{biblatex} or
\pkg{natbib}. Locators are
rendered as a superscript/subscript pair, visually set apart from the
citation number, and every key with its locators is passed in a single
comma-separated argument rather than nested optional arguments. Ten
built-in prefix types, extensible via \cmd{citemspprefix}, give authors a
consistent, per-citation locator registry that neither backend provides
on its own.
...
\end{tcolorbox}
%
\noindent The \verb|kpsewhich| command ships with every TeX Live and MacTeX
installation, so the above works on Linux, macOS, and Windows alike.
Typical resolved paths: \verb|~/texmf| (Linux), \verb|~/Library/texmf|
(macOS/MacTeX), and \verb|C:\Users\<name>\texmf| (Windows/TeX Live).
Using the \verb|kpsewhich|-derived value keeps a single command portable
...
The package must be loaded \emph{after} the citation backend.

\medskip
\noindent\textbf{With biblatex:}
%
\begin{tcolorbox}[colback=gray!5, colframe=gray!40, boxrule=0.5pt, arc=2pt,
  top=6pt, bottom=6pt, left=10pt, right=10pt]
\begin{verbatim}
...
The simplest approach is \verb|latexmk -pdf <file>.tex|, which
auto-detects whether to run \pkg{biber} or \pkg{bibtex} and executes as
many passes as needed.  For manual compilation:

\begin{tcolorbox}[colback=gray!5, colframe=gray!40, boxrule=0.5pt, arc=2pt,
  top=6pt, bottom=6pt, left=10pt, right=10pt]
\begin{tabular*}{\dimexpr\linewidth-0pt}{@{}l@{\extracolsep{\fill}}l@{}}
\textbf{biblatex} & \verb|pdflatex| $\to$ \verb|biber| $\to$ \verb|pdflatex| $\to$ \verb|pdflatex| \\[3pt]
\textbf{natbib}   & \verb|pdflatex| $\to$ \verb|bibtex| $\to$ \verb|pdflatex| $\to$ \verb|pdflatex|
\end{tabular*}
\end{tcolorbox}

\noindent\textbf{Common pitfall:} \pkg{biblatex} with
\texttt{backend=biber} requires \verb|biber|, not \verb|bibtex|.  Running
\verb|bibtex| on a \pkg{biber}-based document will silently produce an
empty bibliography, because the two programs use incompatible
...
All ten built-in prefixes are listed below, in table 1. Note that no prefix defaults to \S\ (section) in the superscript slot and \P\ (paragraph) in the subscript slot. A prefix is a single letter at the start of a locator slot that selects a symbol:
...
Three user-configurable parameters control the citation appearance:
\begin{itemize}[leftmargin=2em]
  ...
  \item \cmd{citemspfixedfonttrue} --- when enabled, the entire
    \cmd{citemsp} output is set in \cmd{textnormal} (upright, medium
    weight, default family) regardless of the surrounding font context.
    Disabled by default, meaning citations inherit the surrounding font.
\end{itemize}
...
\subsection{Superscript-only locator}
...
\begin{tcolorbox}[colback=blue!3, colframe=blue!30, boxrule=0.5pt, arc=2pt,
  top=6pt, bottom=6pt, left=10pt, right=10pt]
\begin{verbatim}
\citemsp{smith2003, wang2020, meyerrochow2003, upper1974, pennycook2015}
\end{verbatim}
\end{tcolorbox}
...
\begin{tcolorbox}[colback=blue!3, colframe=blue!30, boxrule=0.5pt, arc=2pt,
  top=6pt, bottom=6pt, left=10pt, right=10pt]
\begin{verbatim}
\citemsp{upper1974, smith2003/4.1, smith2003, meyerrochow2003, wang2020}
\end{verbatim}
\end{tcolorbox}
...
\begin{itemize}[leftmargin=2em]
  ...
  \item \textbf{Paragraph counting:} count paragraphs from the start of
    the referenced section; as a manual convention.
