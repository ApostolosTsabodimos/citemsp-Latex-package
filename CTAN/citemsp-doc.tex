...
\end{tcolorbox}
%
\noindent \verb|TEXMFHOME| resolves to \verb|~/texmf| on a vanilla TeX Live
install (e.g., Linux) and to \verb|~/Library/texmf| on MacTeX --- using the
\verb|kpsewhich|-derived value keeps a single command portable across both.
...
\subsection{Loading and compiling with biblatex}
...
\end{tcolorbox}
%
\noindent Build with \verb|latexmk -pdf <file>.tex|, or manually with the
chain \verb|pdflatex| $\to$ \verb|biber| $\to$ \verb|pdflatex| $\to$
\verb|pdflatex|. 

\noindent Running plain \verb|bibtex| against a \texttt{backend=biber}
document leaves the bibliography empty, since \pkg{biblatex}/\pkg{biber}
write a different auxiliary-file format than classic \pkg{bibtex} expects.

\subsection{Loading and compiling with natbib}
...
%
\noindent Build with \verb|latexmk -pdf <file>.tex|, or manually with the
chain \verb|pdflatex| $\to$ \verb|bibtex| $\to$ \verb|pdflatex| $\to$
\verb|pdflatex|.
...
\subsection{Source-level comparison}

The difference in source verbosity becomes clear when multiple locator-bearing
citations appear together.  Consider six sources:
...
\autocites[§3.2]{smith2003}[¶2]{meyerrochow2003}
  [eq.\,6.55]{matthews1995}[§8.4, tab.\,2]{wang2020}
  [§2.3]{ghirlanda2002}[§3]{zongker2006}

% citemsp equivalent (6 sources, ~114 characters):
\citemsp{smith2003/3.2, meyerrochow2003//2,
  matthews1995/e6.55, wang2020/8.4/t2,
  ghirlanda2002/2.3, zongker2006/3}
\end{verbatim}
\end{tcolorbox}
...
biblatex: & \autocites[§3.2]{smith2003}[¶2]{meyerrochow2003}[eq.\,6.55]{matthews1995}[§8.4, tab.\,2]{wang2020}[§2.3]{ghirlanda2002}[§3]{zongker2006} \\[4pt]
citemsp:  & \citemsp{smith2003/3.2, meyerrochow2003//2, matthews1995/e6.55, wang2020/8.4/t2, ghirlanda2002/2.3, zongker2006/3}
\end{tabular}

\medskip
\noindent At six entries the \pkg{biblatex} source is roughly 40\% longer, and
the rendered bracket sequence occupies significant horizontal space.  The
\pkg{citemsp} output remains a single compact bracket with superscript/subscript
annotations.
...
\begin{itemize}[leftmargin=2em]
  ...
  \item \textbf{Paragraph counting:} count paragraphs from the start of
    ...
    \emph{Note:} Paragraph locators rely on manual counting from the
    start of the referenced section.  Since paragraphs are not explicitly
    numbered in published sources, these locators may be ambiguous across
    editions or between readers.  When using \P\ locators, consider
    providing additional context in the surrounding text so the reader
    can locate the passage independently.
